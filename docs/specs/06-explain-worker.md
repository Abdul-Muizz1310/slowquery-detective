...
5b. **Read-only guard (hard, before any EXPLAIN)** — `EXPLAIN (ANALYZE)` *executes* its target. Since synthesized parameters would otherwise run a captured `UPDATE users SET active = true WHERE id = 1` against live data, the worker parses each canonical statement (`is_read_only_sql`, sqlglot AST, default-deny) and drops anything that is not a provably read-only query — `INSERT`/`UPDATE`/`DELETE`/`MERGE`, data-modifying CTEs, `DO`/`CALL`, DDL, and unparseable text. Dropped statements set the cooldown and never touch the database via EXPLAIN. They remain fingerprinted and tracked in the ring buffer.
